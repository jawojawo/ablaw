        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editAdminDepositModalLabel"> <span
                        class="adminDepositIdHeader text-primary  px-3"></span> Admin Deposit</h5>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" form="editAdminDepositForm">Save Admin Deposit</button>
            </div>
        </div>
    </div>
</div>
<script>
    $(document).ready(function() {
        $('#editAdminDepositModal').on('show.bs.modal', function(e) {
            ...
            var paymentTypeId = $(e.relatedTarget).data('admin-deposit-payment-type-id')
            var amount = $(e.relatedTarget).data('admin-deposit-amount')
            var depositDate = $(e.relatedTarget).data('admin-deposit-deposit-date')
        ...
        $('#editAdminDepositForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ route('case.updateAdminDeposit', '') }}/' + $(this).find(
